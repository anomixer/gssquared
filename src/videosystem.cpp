...
#ifndef __EMSCRIPTEN__
    // Native platforms: set size constraints and aspect ratio.
    // WASM: skip these completely - any size constraints or aspect ratio
    // enforcement interferes with browser layout and causes clipping.
...
void video_system_t::render_frame_rect(SDL_Texture *texture, SDL_FRect *srcrect,
        const SDL_FRect *dstrect, bool respect_mode) {
    if (!dstrect) {
        render_frame(texture, srcrect, nullptr, respect_mode);
        return;
    }
    if (respect_mode) {
        if (display_pixel_mode == DM_PIXEL_FUZZ) {
