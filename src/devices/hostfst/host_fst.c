...
/*
 * MFS/HFS classic limit. Volume/dir entries report mfsFSID; Finder assumes
 * <=31 character names. Returning longer host names (JudgeName used to
 * advertise 255) can overflow Finder and BRK when opening :Host.
 */
#define HOST_FST_MAX_NAME_LEN 31
...
void host_fst_remount(void) {
  /* Same host-side teardown as fst_startup, without waiting for GS/OS $8001. */
  struct fd_entry *head = fd_head;
  while (head) {
    struct fd_entry *next = head->next;
    free_fd(head);
  ...
  word32 rv = host_startup();
  if (rv) {
    fprintf(stderr, "Host FST: remount failed (%02x %s) path=%s\n",
            (unsigned)rv, host_error_name((word16)rv),
            g_cfg_host_path ? g_cfg_host_path : "(null)");
  } else {
    fprintf(stderr, "Host FST: remounted at %s\n", host_root ? host_root : "(null)");
  }
}
...
static word32 fst_judge_name(int class, char *path) {
  ...
  /* Classic MFS/HFS name limit (matches volume fileSysID / Finder). */
  if (pcount >= 4) set_memory16_c(pb + JudgeNameRecGS_maxLen, HOST_FST_MAX_NAME_LEN, 0);
  ...
  if (name) {
    ...
    if (length == 0) {
      ...
      if (length > HOST_FST_MAX_NAME_LEN) nameFlags |= 1 << 14;
      for (int i = 0; i < length; ++i) {
...
static int filter_directory_entry(const char *name) {
  ...
  /* Skip rather than truncate: truncated names would not open on the host. */
  if (strlen(name) > HOST_FST_MAX_NAME_LEN) {
    HOST_FST_LOG("Host FST: skip long name (%zu > %d): %s\n",
                 strlen(name), HOST_FST_MAX_NAME_LEN, name);
    return 1;
  }
  /* GS/OS path separator — not valid in a single path component. */
  if (strchr(name, ':') != NULL || strchr(name, '/') != NULL) {
    HOST_FST_LOG("Host FST: skip name with path separator: %s\n", name);
    return 1;
  }
  return 0;
}
...
void host_fst(void) {
  ...
  HOST_FST_LOG("Host FST: %04x %s", call, call_name(call));
  ...
  if (call & 0x8000) {
    ...
    if (path1) HOST_FST_LOG(" - %s", path1);
    if (path2) HOST_FST_LOG(" - %s", path2);
