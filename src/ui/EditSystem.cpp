...
EditSystem::EditSystem(video_system_t *vs, AssetAtlas_t *aa)
    ...
    // On WASM, sync with the actual canvas size before setting logical presentation.
...
void EditSystem::open_premount_dialog(storage_key_t key) {
    ...
    static const SDL_DialogFileFilter filters[] = {
        {"Disk Images", "do;po;woz;dsk;hdv;2mg;img;hda;iso"},
        {"All files", "*"},
    };
...
#if defined(__EMSCRIPTEN__)
    web_open_file_dialog(callback, data, ".do,.po,.woz,.dsk,.hdv,.2mg,.img,.hda,.iso");
#else
