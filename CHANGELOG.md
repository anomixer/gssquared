...
### WebAssembly Selector & Improvements
...
- **Final Second Sight canvas layout.** Corrected the SS display model: the WASM canvas remains a maximum `1288x928` surface like the native build, while the native `720x400` Second Sight image is scaled proportionally inside it. The web shell scales the whole canvas down when the viewport is smaller, and the renderer subtracts the in-canvas File/Edit/Machine menu bar before placing the SS image so the top `Apple IIgs` row is not covered. Also added an explicit destination-rectangle render path instead of relying on symmetric letterbox margins.
- **Select / Edit screen mouse alignment & tooltips.** Root cause was `SDL_ConvertEventToRenderCoordinates()` mapping pointer events with a wrong DPR/internal-dst rect on the Emscripten canvas, which compressed the design-space Y axis — the top machine cards still hovered but the bottom `+ New` / `Launch` / `Edit` buttons (large design-Y) never matched, and `SystemButton` machine-description tooltips plus the footer hints never appeared. Reimplemented the inverse-Letterbox transform in **window-points space** (using `SDL_GetWindowSize` + the 1288×928 design aspect), which is DPR-independent and exactly inverts SDL's logical presentation. Applied identically in `SelectSystem::event()` and `EditSystem::event()`. Native builds still use `SDL_ConvertEventToRenderCoordinates`. **Verified (2026-08-18):** the `+ New` / `Launch` / `Edit` footer buttons now highlight on hover; machine-card tooltips and Second Sight Text aspect still pending further confirmation.
- **Second Sight Text display resolution.** When `Display → Second Sight Text` is enabled, the web canvas switches its aspect ratio to the text's native 720×400 (restored to 1288×928 when disabled) so the Apple IIgs text fills the screen at 1:1 without letterbox bars. `transition_to_shutdown()` restores the default aspect when returning to the selector, and `transition_to_emulation()` now calls the same sync on entry so a SS-Text setting saved from a previous run takes effect immediately (no manual off/on needed). On windows wider than the design width the canvas is capped at 1288px (width-driven `min(100vw,1288px)` with `height:auto`), so the text stays crisp and 1:1 and does not balloon to the full viewport height on large browser windows; the 720:400 target is still applied via `video_system_t::forced_target_aspect` / `set_target_aspect()`.

## 2026-08-15

### WebAssembly Bug Fixes

- **Apple IIgs Control Panel shortcut.** Added a clickable `(IIgs) Enter Control Panel` action below the Apple II Hotkeys reference. It queues the equivalent `Ctrl + Left Alt + Esc` SDL/ADB sequence, avoiding the browser's `Ctrl + Esc` shortcut conflict. `runweb.bat` now rebuilds the web target before starting the local server so embedded shell changes are immediately visible.
- **Second Sight Text aspect ratio.** Fixed `Display -> Second Sight Text` stretching the `720x400` VGA text surface over the full `1288x928` target, which pushed the Apple IIgs title toward the top edge or clipped it. The text surface now preserves its aspect ratio and is vertically centered.
- **Web shell header cleanup.** Removed the non-functional drive LEDs and Drives button. Moved runtime and media status messages from the lower-left corner to the centered header status area.
- **Close Emulation canvas lifecycle.** Fixed stale Emscripten pointer listeners surviving an in-page return to Select Machine. The web shell now replaces the canvas before the old SDL window is destroyed, preventing `querySelector('')` and WASM memory-access exceptions. WASM keeps SDL alive during the in-page transition and releases mouse capture first.
- **Resize-safe selector menu.** Fixed File/Edit/Machine menu text being covered or clipped after browser resizing. The selector/editor menu is presented in the actual window coordinates, and the frame is presented while the menu uses `STRETCH`; the fixed `1288x928 LETTERBOX` presentation is restored only after `SDL_RenderPresent()` so SDL's letterbox borders cannot overwrite the menu.

## 2026-08-14

### Features & WebAssembly Improvements

- **Web UI Menu Alignment and Official Branding.** Rendered the selector/editor menu overlay in window coordinates so mouse hit-testing and text share the same transform on GitHub Pages. Applied the window-points-to-pixel-output renderer scale during menu drawing so submenu text is not clipped at non-1:1 browser DPI. Replaced the web header logo, startup badge, favicon, and PWA manifest icon with the official square GS2 mark (`assets/img/gssquared-mark.png`), and added it to the Emscripten deploy assets.
- **WASM Mouse Capture Recovery.** Kept the File/Edit/Machine menu visible while mouse capture is active, preserved `F1` as the Capture / Release toggle without consuming `Esc`, and added a browser Pointer Lock retry on the next canvas click when the initial menu-triggered request is rejected because it is outside a user gesture.

- **Modern Glassmorphism Web UI / Shell.** Redesigned `assets/web/shell.html` with a modern dark glassmorphism layout, 'GSSquared Web' branding, GitHub repository link (`anomixer/gssquared`), responsive navigation bar, active drive LEDs (`S6D1`, `S6D2`, `HD1`), Drive Mount modal, Apple II Hotkeys reference modal, audio mute toggle, and fullscreen toggle.
- **IndexedDB Storage Persistence.** Integrated Emscripten IDBFS persistence mapped to `/persistent/` in `src/paths.cpp` and `assets/web/shell.html` so NVRAM/PRAM, configuration files, and saved disk images survive browser reloads.
- **PWA (Progressive Web App) & Offline Support.** Added `assets/web/manifest.json`, Service Worker `assets/web/sw.js`, and `assets/web/coi-serviceworker.js` for Cross-Origin Isolation (COOP/COEP) on GitHub Pages to enable `SharedArrayBuffer` and `-pthread` WASM loading without asset freezes.
- **WASM SIMD Support.** Added `GS2_WASM_SIMD` CMake build option for WebAssembly SIMD optimizations (`-msimd128`).
- **WebAudio Latency Fix.** Resolved 15-20 second audio buffer delay issue in `src/devices/es5503/soundglu.cpp`, `mb.cpp`, and `mb2.cpp` by automatically purging stale audio queue backlog accumulated while WebAudio is suspended or delayed.
- **Initial Resolution Auto-Detection & Web Layout Fix (Complete Root Cause Fix).** Fixed all root-cause layout clipping, font rendering quality, and mouse hover alignment on initial web startup in `assets/web/shell.html`, `src/videosystem.cpp`, `SelectSystem.cpp`, and `EditSystem.cpp` by removing hardcoded canvas width/height HTML attributes to allow SDL3 native DPI auto-sizing, eliminating JS canvas pixel mutations to prevent WebGL context loss loops, aligning SDL3 logical presentation matrix updates with window resize events, and removing viewport overrides; added a 5-second auto-dismiss timer to the start overlay screen. **Root cause fix: removed forced window sizing in WASM**: Modified `videosystem.cpp` to create window with 0x0 dimensions (container-adaptive) on WASM and skip `SDL_SetWindowMinimumSize`/`SDL_SetWindowAspectRatio` constraints, letting HTML/CSS control canvas layout entirely; native builds retain fixed 1288x928 window with aspect ratio enforcement. **Per-render synchronization**: Modified `SelectSystem::ensure_logical_presentation()` and `EditSystem::ensure_logical_presentation()` to read actual renderer output size and update target rect before every `SDL_SetRenderLogicalPresentation` call (in both `event()` and `render()`), ensuring canvas-to-SDL sync at all times. Modified `shell.html` to defer resize triggers until after overlay dismissal, preventing rendering at incorrect dimensions during startup. Multi-stage resize events (0ms, 16ms, 100ms, 200ms) after overlay removal ensure complete browser layout stabilization. **[2026-08-14 Late] Debug logging investigation**: Added comprehensive debug logging to `SelectSystem.cpp`, `EditSystem.cpp`, and `videosystem.cpp` to track canvas size detection timing and `SDL_SetRenderLogicalPresentation` synchronization; logs show canvas dimensions at constructor, `ensure_logical_presentation()` calls, and `update_target_from_output()` to diagnose remaining clipping issues during initial load and after close emulation scenarios.
- **WebAssembly Clean Emulation Exit Fix.** Fixed `Exception thrown, see JS console` crash when selecting `File -> Close Emulation` or `File -> Quit` in `src/gs2.cpp` by handling `SDL_EVENT_QUIT` in emulation phase and transitioning back to system selector UI without throwing WebAssembly C++ exit exceptions.
- **Developer Convenience Scripts.** Added `buildweb.bat` (automated Emscripten setup + WASM build) and `runweb.bat` (`python assets/web/serve.py 8000 build-web`) for one-click local WebAssembly building and serving.
- **Web Startup Overlay UX.** Removed the manual `Start Emulator` action so users cannot enter before the browser canvas layout is stable. The startup overlay now remains visually unchanged while the WASM runtime initializes and auto-starts after the five-second delay, preventing an unnecessary intermediate shell redraw. Added explicit resize notifications after renderer recreation on `Close Emulation`, and made `buildweb.bat` clean/relink the web target so the embedded Emscripten shell cannot remain stale.
- **WASM Runtime and Disk Menu Fixes.** Kept presenting the current frame while emulation is paused so Emscripten's `requestAnimationFrame` loop remains alive and `Machine → Pause / Resume` does not freeze the browser. Fixed `File → Drives` on wasm32 by packing slot/drive fields instead of truncating the 64-bit storage key through a pointer. Disabled the second asset-cache service worker on GitHub Pages and added cleanup of existing registrations to prevent refreshes from mixing `.wasm` and `.data` versions at `Loading assets... (67/68)`.
